...
class Admin extends CI_Controller {
			...
			//die();
				...
				//die();
					...
					elseif($user['role']  == 2)
					{
						...
						$this->session->set_userdata('clientID' , $user['id']);	
						$this->session->set_userdata('clientName' , $user['username']);
						$this->session->set_userdata('clientRole' , $user['role']);
	...
	public function logout()
    {
		...
		$this->session->unset_userdata('adminID');
		//session_destroy();
		$this->session->set_flashdata('item', array('message' => 'Successfully Logout','class' => 'error'));
